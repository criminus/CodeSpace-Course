...
)
{
    ...
    $stmt = $pdo->prepare('SELECT first_name, last_name, email FROM users WHERE first_name = ? AND last_name = ? AND email = ?');
    $stmt->execute([$first_name, $last_name, $email]);
    $userCheck = $stmt->fetch(PDO::FETCH_ASSOC);

    //Check here
    if ($userCheck) {
        $errors[] = 'There is already a customer with this details.';
    }
